...
const int NUM_TO_SMOKE = 10; // Number of times to smoke
...
void agent()
{
	...
	{
		// Chooses a random smoker to deliver items to
		int items = rand() % 3;
		// Rerolls if that smoker has smoked their max amount
		while (numTimesSmoked[items] >= NUM_TO_SMOKE)
			items = rand() % 3;
		...
		// Wait for a smoker to grab the items
...
{	
	...
	WaitForSingleObject(mutex, INFINITE);
	cout << "Smoker " << myID << " done." << endl << endl;
	ReleaseMutex(mutex);
}
